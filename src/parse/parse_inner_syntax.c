...
/*   Updated: 2023/05/21 13:28:40 by jaemjeon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_squence_subshell(t_lex_lst *curr)
{
	t_s_lex	*front_lex_data;
	t_s_lex	*rear_lex_data;
	...
	while (curr && curr->next)
	{
		rear_lex_data = curr->content;
		front_lex_data = curr->next->content;
		if (rear_lex_data->type == LEXEME_PARENTHESIS_CLOSE
			&& front_lex_data->type == LEXEME_PARENTHESIS_OPEN)
			return (1);
		curr = curr->next;
	}
	return (0);
...
int	check_inner_syntax(t_lex_lst *curr)
{
	...
	if (check_empty_subshell(curr))
		return (1);
	if (check_squence_subshell(curr))
		return (1);
	return (0);
}
